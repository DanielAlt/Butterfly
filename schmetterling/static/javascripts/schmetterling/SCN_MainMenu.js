...
var MainMenu = function(myMenu){

	myMenu.spawn = function(globalPersistence){
		var bf_img = new Poster.spawn();
		var globalPersistence = globalPersistence;

		var menu_options = new ListBox.spawn({
			'options': {
				'New Game' : function(){
					globalPersistence.args['manager'].loadScene('game');
				},
				'Options': function(){
					globalPersistence.args['manager'].loadScene('options');
				},
				'Credits': function(){
					globalPersistence.args['manager'].loadScene('credits');
				},
				'Quit' : function(){
			...
			$(window).keydown(function(e){
				...
				switch(e.keyCode){
					...
					case 13:
						menu_sfx.play();
						ambient_sound.stop();
						key = menu_options.getValue();
						menu_options.args['options'][key]();
						break;
				};
			});
		};
		...
		this.render = function(screen){
			...
			// Render Picture
			if (bf_img.loaded){
				bf_img.render(screen);
				screen.fillText("Schmetterling", 30, 75);
				menu_options.render(screen);
			} else {
				screen.fillText("Loading", 200,200);
			};
		};
	};
	return (myMenu);
